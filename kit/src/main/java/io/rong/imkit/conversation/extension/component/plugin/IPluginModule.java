...
public interface IPluginModule {
    ...
    /**
     * /~english
     * Called when plugin is clicked. 1. If you need the data in Extension, you can call the corresponding method of Extension to get it.. 2. If you shall open a new activity, after clicking, you can use Activity.startActivityForResult(Intent, int) or RongExtension.startActivityForPluginResult(Intent, int, IPluginModule) .
     * Note: Do not hold fragment or extension objects for a long time, or there will be a memory leak.
     *
    ...
    /**
     * /~english
     * The data result is returned at the end of the activity.
     * In onClick(Fragment, RongExtension, int) , you may open a new activity,. You can turn it on in two ways:
     * 1. Using the Activity.startActivityForResult(Intent, int) opening method in the system, you shall receive the results returned by the processing Activity.onActivityResult(int, int, Intent) in the corresponding Activity.
     * 2. If the RongExtension.startActivityForPluginResult(Intent, int, IPluginModule) open method is called, after receiving the Activity.onActivityResult(int, int, Intent) in ConversationFragment, the method RongExtension.onActivityPluginResult(int, int, Intent) must be called and then RongExtension will return the data result through the onActivityResult method in IPluginModule.
     *
