...
public class RongIM {
     ...
     * <p>After the other party is added into the blacklist, when the other party sends a message again, the system prompts "your message has been sent,
     * but it has been rejected by the other party." But you can still send messages to each other.</p>
     *
    ...
    /**
     * /~english
     * Set unread message number change listener Note: If it is set in activity, then call UnReadMessageManager.removeObserver(UnReadMessageManager.IUnReadMessageObserver) when the activity is destroyed, otherwise it will cause a memory leak
     *
    ...
    /**
     * /~english
     * Cancel multimedia message downloading.
     *
    ...
    /**
     * /~english
     * Cancel sending multimedia files.
     *
     * @param message  Multimedia file message.
    ...
    /**
     * /~english
     * Clears all conversations and conversation information in the list of specified conversation types.
     *
     ...
     * <p><strong>Note: The chat rooms are not supported!</strong></p>
     *
     * @param conversationType Type of conversation, which does not support chat rooms. Refer to Conversation.ConversationType.
     * @param targetId         Conversation id. Depending on different conversationType, it may be user id, discussion group id or group id.
    ...
    /**
     * /~english
     * Clear unread messages in a conversation.
     * <p><strong>Note: The chat rooms are not supported!</strong></p>
     *
     * @param conversationType Type of conversation, which does not support chat rooms. Refer to Conversation.ConversationType.
     * @param targetId         Conversation id. Depending on different conversationType, it may be user id, discussion group id or group id.
    ...
    /**
     * /~english
     * Delete draft information in a specified conversation.
     *
     * @param conversationType Conversation type
     * @param targetId         Conversation id. Depending on the conversationType, it may be user id, discussion group id, group id, or chatroom id.
    ...
    /**
     * /~english
     * Initialize the RongCloud SDK, which only needs to be called once in the whole application globally.
     * <p>It can only be called in the main process and it is recommended to call in the Application inheritance class.</p>
     *
     * @param application Application class object.
     * @param appKey      The App Key obtained after an application is created on RongCLoud developer platform.
     */
    ...
    /**
     * /~english
     * SDK Initialization shall be called only once globally throughout the application, and it is recommended to call in the Application inheritance class.
     *
     * @param application Application class object.
     * @param appKey      The App Key obtained after an application is created on RongCLoud developer platform.
     *                    This parameter is optional. If you do not pass a value, SDK will use the RONG_CLOUD_APP_KEY configured in AndroidManifest.xml.
     *                    <meta-data android:name="RONG_CLOUD_APP_KEY" android:value="******" />
     ...
     * 向本地会话中插入一条消息，方向为接收。
     * <p>
     * 这条消息只是插入本地会话，不会实际发送给服务器和对方。插入消息需为入库消息，即 {@link MessageTag#ISPERSISTED}，否者会回调 {@link io.rong.imlib.IRongCoreEnum.CoreErrorCode#PARAMETER_ERROR}。
     * </p>
     *
     * @param type           会话类型。
    ...
    /**
     * /~english
     * Insert a message into the local conversation in the direction of receiving
     * <p>his message will only be inserted into the local conversation and will not actually be sent to the server and the other party.
     * The message to be inserted must be stored into the database, namely {@link MessageTag#ISPERSISTED}, otherwise {@link io.rong.imlib.IRongCoreEnum.CoreErrorCode#PARAMETER_ERROR} will be called back.</p>
     *
     * @param type           Conversation type
     * @param targetId       Conversation id. For example: It is an id of other party for a private conversation, the group id for a group conversation, and the id of the discussion group for a discussion group conversation.
     ...
     * 插入消息需为入库消息，即{@link MessageTag#ISPERSISTED}，否者会回调 {@link io.rong.imlib.IRongCoreEnum.CoreErrorCode#PARAMETER_ERROR}
     * </p>
     *
     * @param type           会话类型。
     * @param targetId       会话 id。比如私人会话时，是对方的 id； 群组会话时，是群 id； 讨论组会话时，则为该讨论组的 id。
    ...
    /**
     * /~english
     * Insert a message into the local conversation in the direction of sending
     * <p>his message will only be inserted into the local conversation and will not actually be sent to the server and the other party.
     * The message to be inserted must be stored into the database, namely {@link MessageTag#ISPERSISTED}, otherwise {@link io.rong.imlib.IRongCoreEnum.CoreErrorCode#PARAMETER_ERROR} will be called back.</p>
     *
     * @param type       Conversation type
     ...
     * 加入聊天室。
     * <p>
     * 如果聊天室不存在，SDK 会创建聊天室并加入，如果已存在，则直接加入。 <br>
     ...
     * /~english
     * Join the chat room
     * <p>If a chat room does not exist, SDK creates a chat room and joins it, or directly joins if it already exists.
     * You can use the passed defMessageCount to set the number of historical messages that shall be obtained after joining the chat room successfully.
     * - 1 indicates no historical messages are obtained, 0 means to use the default setting of SDK without special settings (default value is to get 10),
     * defMessageCount (0 < defMessageCount <= 50) is totally obtained message amount, maximum 50.</p>
     *
     * @param chatRoomId      Chat room id
     * @param defMessageCount Enter the chat room to pull the number of messages. No messages are pulled at-1, 10 messages are pulled at 0, and a maximum of 50 messages can be pulled (The timestamp of the last local message will be sent when you join the chat room, and the message will be pulled after this timestamp. For example, there are 3 messages after this timestamp, and defMessageCount sends 10, and only 3 messages can be pulled. )
     ...
     * <p>If the chat room does not exist, it fails to join. When you join a chat room, you can set the number of messages to be pulled from the chat room.</p>
     *
     * @param chatroomId      Chat room id
     * @param defMessageCount Enter the chat room to pull the number of messages. No messages are pulled at-1, 10 messages are pulled at 0, and a maximum of 40 messages can be pulled.
     * @param callback        Status callback.
     */
    ...
    /**
     * /~english
     * Disconnect from the RongCloud server and no longer receive remote push messages.
     * <p>If you want to receive remote push messages after disconnection, you can call {@link #disconnect()}</p>
     */
    public void logout() {
    ...
    /**
     * /~english
     * Pause downloading of multimedia messages.
     *
     * @param message  Multimedia file message.
    ...
    /**
     * /~english
     * Refresh and change group user cache data
     *
    ...
    /**
     * /~english
     * Refresh public service account cache data
     *
    ...
    /**
     * /~chinese
     * 连接融云服务器，在整个应用程序全局，只需要调用一次，需在 {@link #init(Application, String, boolean)} 之后调用。
     * <p>
     * 调用此接口返回非业务错误码时，SDK 会启动重连机制进行重连；如果仍没有连接成功，会在设备网络状态变化时再次进行重连。<br>
     * <strong>如果您使用 IMKit，请使用 RongIM 中的同名方法建立与融云服务器的连接，而不要使用此方法。</strong>
     * </p>
     *
     * @param token           从服务端获取的用户身份令牌（Token）。
     * @param connectCallback 连接回调扩展类，新增打开数据库的回调（OnDatabaseOpened(DatabaseOpenStatus)），会返回数据库打开的状态，{@link DatabaseOpenStatus}；
     *                        {@code DATABASE_OPEN_SUCCESS} 执行拉取会话列表操作，{@code DATABASE_OPEN_ERROR} 不处理。<br>
    ...
    /**
     * /~english
     * Connect to the RongCloud server, which only needs to be called once in the whole application globally, and needs to be called after {@link #init(Application, String, boolean)} .
     * <p>When a non-business error code is returned by calling this API, SDK will activate the reconnection mechanism for reconnection; if the connection is still not successful, it will reconnect again when the network status of the device changes.
     * If you use IMKit, the method of the same name in RongIM is used to establish a connection to the RongCloud server instead of using this method.</p>
     *
     * @param token           User identity token (Token) obtained from the server.
     * @param connectCallback Connection callback extension class. When the new callback (OnDatabaseOpened (DatabaseOpenStatus)) to open the database is added, it will return the open status of the database.DatabaseOpenStatus; DATABASE_OPEN_SUCCESS exepull the conversation list and DATABASE_OPEN_ERROR is not handled.
     *                        If the connection fails, see the specific description of ConnectionErrorCode error code.
    ...
    /**
     * /~chinese
     * 连接融云服务器，在整个应用程序全局，只需要调用一次，需在 {@link #init(Context)} 之后调用。
     * <p>
     * 调用此接口返回非业务错误码时，SDK 会启动重连机制进行重连；如果仍没有连接成功，会在设备网络状态变化时再次进行重连。<br>
     * <strong>如果您使用 IMKit，请使用 RongIM 中的同名方法建立与融云服务器的连接，而不要使用此方法。</strong>
     * </p>
     ...
     * 第一、连接成功，回调 onSuccess(userId)。
     * 第二、超时，回调 onError(RC_CONNECT_TIMEOUT)。
     * 第三、出现 SDK 无法处理的错误，回调 onError(errorCode)（如 token 非法）。
     * @discussion 连接成功后，SDK 将接管所有的重连处理。当因为网络原因断线的情况下，SDK 会不停重连直到连接成功为止，不需要您做额外的连接操作。
     * @group 连接
     */
    ...
    /**
     * /~english
     * Connect to the RongCloud server, which only needs to be called once in the whole application globally, and needs to be called after init(Context).
     * <p>When a non-business error code is returned by calling this API, SDK will activate the reconnection mechanism for reconnection; if the connection is still not successful, it will reconnect again when the network status of the device changes.
     * If you use IMKit, the method of the same name in RongIM is used to establish a connection to the RongCloud server instead of using this method.</p>
     *
     * @param token           User identity token (Token) obtained from the server.
     * @param timeLimit       Connection timeout (in seconds). timeLimit <= 0, then Im will continue to connect until the connection is successful or unable to connect (for example, the token is illegal) with timelimit > 0, then Im will connect with timelimit seconds at most: If the connection is successful within timeLimit second(s), the SDK will automatically reconnect if there is a network change or a switch between the foreground and background. If the connection cannot be successfully connected in timeLimit second(s), the connection will not be reconnected. You shall call the connect interfaceagain when you are told by onError that the connection has timed out.
     * @param connectCallback Connection callback extension class. When the new callback (OnDatabaseOpened (DatabaseOpenStatus)) to open the database is added, it will return the open status of the database.DatabaseOpenStatus; DATABASE_OPEN_SUCCESS exepull the conversation list and DATABASE_OPEN_ERROR is not handled.
     *                        If the connection fails, see the specific description of ConnectionErrorCode error code.
     * @return Instance of the RongCoreClient IM client core class.
     */
     ...
     * 删除某个会话中的所有消息。
     * <p>
     * 此接口删除指定会话中数据库的所有消息，同时会清理数据库空间，减少占用空间。<br> </p>
     *
     * @param conversationType 会话类型，不支持聊天室。参考 {@link Conversation.ConversationType} 。
     ...
     * /~english
     * Delete all messages in a conversation.
     * <p>This interface deletes all messages from the database in the specified conversation while cleaning up the database space and reducing the footprint.</p>
     *
     * @param conversationType Type of conversation, which does not support chat rooms. Refer to Conversation.ConversationType.
     * @param targetId         Conversation id. Depending on different conversationType, it may be userId, groupId or discussionId.
     ...
     * <p>
     * 一次批量操作仅支持删除属于同一个会话的消息，请确保消息列表中的所有消息来自同一会话，一次最多删除 100 条消息。<br>
     * <strong>注意：不支持聊天室！</strong>
     * <p>
     *
     * @param conversationType 会话类型， 不支持聊天室。参考 {@link Conversation.ConversationType} 。
    ...
    /**
     * /~english
     * Deletes specified remote messages in a conversation (while deleting the corresponding local messages) in batches.
     * <p>One batch operation only supports to delete messages belonging to the same conversation, please make sure that all messages in the message list come from the same conversation and delete at most 100 messages at a time.
     * Note: The chat rooms are not supported!</p>
     *
     * @param conversationType Type of conversation, which does not support chat rooms. Refer to Conversation.ConversationType.
     * @param targetId         Conversation id. Depending on different conversationType, it may be user id and customer service id.
     ...
     * <p>If you want not to receive remote push messages after disconnection, you can call {@link #logout()}. <br>
     * <strong>Note：</strong>The system can automatically reconnect to ensure reliability of the connection because SDK switches between the foreground and background or the network is abnormal. <br>
     ...
     * <p>If this file is included in the local cache, it is obtained directly from the local cache, otherwise it will be downloaded from the server side.</p>
     *
     * @param conversationType Conversation type
     * @param targetId         Conversation id. Depending on the conversationType, it may be user id, discussion group id, group id, or chatroom id.
     ...
     * 下载文件。
     * <p>
     * 支持断点续传，对应的暂停下载须调用 {@link #pauseDownloadMediaFile(String, RongIMClient.OperationCallback)}。
     * </p>
     *
     * @param fileUniqueId 文件唯一标识, 与 {@link #pauseDownloadMediaFile(String, RongIMClient.OperationCallback)} 第一个参数对应
     ...
     * /~english
     * Download the file.
     * <p>Resuming downloading from breakpoints is supported and pauseDownloadMediaFile(String, IRongCoreCallback.OperationCallback) shall be called when downloading pauses.</p>
     *
     * @param uid      Unique file identifier, which corresponds to the first parameter of pauseDownloadMediaFile(String, IRongCoreCallback.OperationCallback)
     * @param fileUrl  File download address
     * @param fileName File name
     * @param path     File downloading and saving directory. If targetVersion 29 is the target, it is recommended to use the context.getExternalFilesDir() method to save to a private directory due to access rights
     * @param callback Callback for downloading files
     */
     ...
     * Download multimedia files.
     *
     * @param message  Media messages (FileMessage, SightMessage, GIFMessage, HQVoiceMessage, among others).
     * @param callback Callback for downloading files Refer to {@link IRongCallback.IDownloadMediaMessageCallback}.
     */
    ...
    /**
     * /~english
     * Set whether to display unread new messages in the conversation interface. Note: Unread new messages will be displayed if the number of unread new messages is more than 1. Only single group chat is supported.
     *
    ...
    /**
     * /~english
     * Set whether to display history messages in the conversation interface. Note: History messages will be displayed if there are more than 10 historical messages. Only single group chat is supported.
     *
     * @param state True for display and false for no display
     */
    ...
    /**
     * /~english
     * Set the listener that receives the message
     * All received messages, notifications, and status are processed by the listeners set here It includes private chat messages, discussion group messages, group messages, chat room messages, and various states.
     *
    ...
    /**
     * /~english
     * Remove the listener that receives the message。
     *
     * @param listener Listeners that receive messages
     */
    ...
    /**
     * /~english
     * Set the listener for changes in the state of the connection.
     * <p>When the callback status is RongIMClient.ConnectionStatusListener.ConnectionStatus#TOKEN_INCORRECT the
     * correct token shall be obtained and actively callRongIM.connect(String, int, RongIMClient.ConnectCallback)</p>
     *
    ...
    /**
     * /~english
     * Set the listener for conversation interface operations.
     *
    ...
    /**
     * /~chinese
     * Set the listener for the conversation list interface operation.
     *
     ...
     * @param targetId           会话 id。根据不同的 conversationType，可能是用户 id、讨论组 id、群组 id。
     * @param notificationStatus 会话设置的消息提醒状态 {@link Conversation.ConversationNotificationStatus} 。
    ...
    /**
     * /~english
     * Set the conversation message reminder status.
     * <p><strong>Note: The chat rooms are not supported!</strong></p>
     *
     * @param conversationType   Conversation type The chat room is not supported because the chat rooms do not accept message reminders by default.
     * @param targetId           Conversation id. Depending on different conversationType, it may be user id, discussion group id or group id.
     * @param notificationStatus Message reminder status set for the conversation Conversation.ConversationNotificationStatus.
     ...
     * /~chinese
     * 设置会话的置顶状态。
     * <p>若会话不存在，调用此方法 SDK 自动创建会话并置顶。</p>
     *
     * @param conversationType 会话类型 {@link Conversation.ConversationType} 。
     ...
     * <p>If the conversation does not exist, this method SDK will be called to automatically create the conversation and set top.</p>
     *
     * @param type       Conversation type {@link Conversation.ConversationType}.
     * @param id         Conversation id. Depending on the conversationType, it may be user id, discussion group id, group id, or chatroom id.
    ...
    /**
     * /~english
     * Start the single chat interface
     *
     * @param context      Application context.
     * @param targetUserId Id of the user to chat with
     * @param title        The title of the chat Developers shall get this value in the chat interface, and then manually set it as the title of the chat interface.
     */
    ...
    /**
     * /~english
     * Start the group chat interface
     *
     * @param context       Application context.
     * @param targetGroupId Group Id to chat
     * @param title         The title of the chat Developers shall get this value through intent.getData (). GetQueryParameter ("title") in the chat interface, and then manually set it as the title of the chat interface
     */
     ...
     * @param context          Application context.
     * @param conversationType Conversation type
     * @param targetId         Depending on the conversationType, it may be user Id, discussion group Id, group Id or chat room Id
     * @param title            The title of the chat Developers shall get this value through intent.getData (). GetQueryParameter ("title") in the chat interface, and then manually set it as the title of the chat interface
     */
    ...
    /**
     * /~english
     * Start the conversation interface and jump to the specified message location
     * Multiple conversation types Conversation.ConversationType can be passed in to correspond to different conversation types and open different conversation interfaces in use. If Conversation.ConversationType#CHATROOM is passed in, RongIMClient#joinChatRoom(String, int, RongIMClient.OperationCallback) will be called by default to join the chat room.
     *
     * @param context          Application context.
     * @param conversationType Conversation type
     * @param targetId         Depending on the conversationType, it may be user Id, discussion group Id, group Id or chat room Id
    ...
    /**
     * /~english
     * Get information of a conversation according to the target Id, callback method of different conversation types.
     *
     * @param type     Conversation type
     * @param targetId Target Id Depending on the conversationType, it may be user Id, discussion group Id, group Id, or chatroom Id.
    ...
    /**
     * /~english
     * Get the conversation list according to the conversation type and callback method.
     *
     ...
     * @group 会话
     * @see Conversation.ConversationNotificationStatus
     */

    /**
     * /~english
     * Get the conversation message reminder status.
     * <p><strong>Note: The chat rooms are not supported!</strong></p>
     *
     * @param conversationType Conversation type The chat room is not supported because the chat rooms do not accept message reminders by default.
     * @param targetId         Conversation id. Depending on different conversationType, it may be user id, discussion group id or group id.
     ...
     * 获取当前用户设置的黑名单列表。
     *
     * @param callback 获取黑名单回调。参考 {@link RongIMClient.GetBlacklistCallback} 。
     * @group 高级功能
     */

    /**
     * ~english
     * Get a blacklist set by the current user.
     *
     * @param callback Callback for getting blacklist Refer to {@link RongIMClient.GetBlacklistCallback}.
     */
     ...
     * @group 高级功能
     * @see RongIMClient.BlacklistStatus
     */

    /**
     * /~english
     * Check whether a user is on the blacklist.
     *
     * @param userId   User id.
    ...
    /**
     * /~english
     * Get the time difference between the local time and the server time.
     * <p>After the message is sent successfully, the SDK synchronizes the time with the server, and the time stored in the database where the message is located is the server time.
     * System.currentTimeMillis() - getDeltaTime() can get the current time of the server.</p>
     *
     * @return
     */
     ...
     *
     * @param conversationType Type of conversation, which does not support chat rooms. Refer to Conversation.ConversationType.
     * @param targetId         Conversation id. Depending on differnet conversationType, it may be user id, discussion group id or group id.
     * @param oldestMessageId  Id of the last message. Get the messages of the count value before this message. If there are no messages, it shall be set as -1 for the first call.
     * @param count            Number of messages to get
     * @param callback         Callback for getting historical messages, which are in chronological order from earliest to most recent.
     */
     ...
     * 如：oldestMessageId 为 10，count 为 2，会返回 messageId 为 9 和 8 的 Message 对象列表。<br>
     * </p>
     *
     * @param conversationType 会话类型 {@link Conversation.ConversationType}
     * @param targetId         会话 id。根据不同的 conversationType，可能是用户 id、讨论组 id、群组 id 。
    ...
    /**
     * /~english
     * Gets a list of eligible messages in the conversation.
     * <p>The returned message does not contain the message corresponding to oldestMessageId. If the number of messages in the conversation is less than the value of the parameter count, all messages in the conversation will be returned. For example, if the oldestMessageId is 10 and the count is 2, a list of Message objects with messageId as 9 and 8 will be returned.
     * Note: Do not support to pull the historical messages of the chat room Conversation.ConversationType.CHATROOM</p>
     *
     * @param conversationType Type of conversation Conversation.ConversationType, the chat room is not supported.
     * @param targetId         Conversation id. Depending on differnet conversationType, it may be user id, discussion group id or group id.
     * @param objectName       Identifier of message type
     * @param oldestMessageId  Id of the last message and messages of the count value before this message is got. If there is no message, it should be set to -1 in case of the first call.
     * @param count            Number of messages to get
     * @param callback         Callback for getting historical messages, which are in chronological order from earliest to most recent.
     */
    public void getHistoryMessages(Conversation.ConversationType conversationType, String
    ...
    /**
     * /~english
     * Gets the latest messages from the specified conversation.
     *
     * @param conversationType Conversation type {@link Conversation.ConversationType}.
     * @param targetId         Conversation id. Depending on the conversationType, it may be user id, discussion group id, group id, or chatroom id.
     * @param count            Number of messages to be got.
     * @param callback         Callback for getting the latest messages, which are in chronological order from earliest to most recent. If the number of messages in the conversation is less than the value of the parameter count, all messages in the conversation will be returned.
     */
    ...
    /**
     * /~english
     * Get a list of followed public accounts.
     *
     ...
     * 获取某公共服务信息。
     *
     * @param publicServiceType 公众服务类型。参考 {@link Conversation.PublicServiceType} 。
    ...
    /**
     * /~english
     * Get some public service information.
     *
     * @param publicServiceType Types of public services Refer to {@link Conversation.PublicServiceType}
     ...
     * /~chinese
     * 从服务器端获取指定时间之前的历史消息。
     * <p> 区别于 {@link #getHistoryMessages}，该接口是从融云服务器中拉取。从服务端拉取消息后，客户端会做排重，返回排重后的数据。通常用于更换新设备后，拉取历史消息。<br>
     * 使用的时候，建议优先通过 {@link #getHistoryMessages(Conversation.ConversationType, String, int, int, RongIMClient.ResultCallback)} 从本地数据库拉取历史消息，
     ...
     * @param dateTime         从该时间点开始获取消息。即：消息中的 sentTime {@link Message#getSentTime()}；如果本地库中没有消息，第一次可传 0，否则传入最早消息的sentTime，获取最新 count 条。
    ...
    /**
     * /~english
     * Get historical message before the specified time from the server.
     * <p>It is different from getHistoryMessages(io.rong.imlib.model.Conversation.ConversationType, java.lang.String, int, int). This interface is pulled from the RongCloud server. After messages are pulled from the server, the client will sort them again and return the sorted data. It is usually used to pull historical messages after replacement of new equipment.
     * When it is used, it is recommended to pull the historical messages from the local database through #getHistoryMessages(Conversation.ConversationType, String, int, int, ResultCallback) When there are no historical messages in the local database, you can get the historical message on the server through this interface and the earliest message timestamp in the local database is transferred in.
     * Note:
     * 1. To enable this function, you need to enable the historical message cloud storage function at the backend of the RongCLoud developer platform.
     * 2. This interface does not return data when the message to be fetched already exists in the local database.</p>
     *
     * @param conversationType Conversation type
     * @param targetId         Id of destination conversation. Depending on differnet conversationType, it may be user id, discussion group id or group id.
     * @param dateTime         Get the message from this point in time Namely, sentTime {@link Message#getSentTime()} in the message; 0 can be passed for the first time if there is no message in the local library, otherwise the sentTime of the earliest message is passed to get the latest count messages.
    ...
    public void getRemoteHistoryMessages(Conversation.ConversationType conversationType, String
            targetId, long dateTime, int count, RongIMClient.ResultCallback<List<Message>> callback) {
    ...
    /**
     * /~english
     * Get the remote historical message from the specified conversation.
     * <p>To enable this function, you need to enable the historical message cloud storage function at the backend of the RongCLoud developer platform.
     * Note: The chat rooms are not supported!</p>
     *
    ...
    /**
     * /~english
     * Get draft information in a conversation.
     *
     * @param conversationType Conversation type Refer to {@link Conversation.ConversationType} .
     * @param targetId         Conversation id. Depending on the conversationType, it may be user id, discussion group id, group id, or chatroom id.
     ...
     * 获取指定会话的未读消息数。
     * <p>
     * <Strong>注意：不包含聊天室，聊天室消息不计数。</Strong>
     * </p>
     *
     * @param conversationType 会话类型 {@link Conversation.ConversationType} 。
     * @param targetId         会话 id。根据不同的 conversationType，可能是用户 id、讨论组 id、群组 id。
    ...
    /**
     * /~english
     * Get the number of unread messages for the specified conversation.
     * <p><Strong>Note: Chat rooms are not included and messages in chat rooms are not counted.</Strong></p>
     *
     * @param conversationType Conversation type {@link Conversation.ConversationType}.
     * @param targetId         Conversation id. Depending on differnet conversationType, it may be user id, discussion group id or group id.
    ...
    /**
     * /~english
     * Get the total number of unread messages for the specified conversation type.
     * <p><Strong>Note: Chat rooms are not included and messages in chat rooms are not counted.</Strong></p>
     *
     * @param callback          Callback for the number of unread messages
     ...
     * 获取指定会话类型的总未读消息数。
     * <p>
     * <Strong>注意：不包含聊天室，聊天室消息不计数。</Strong> </p>
     *
     * @param conversationTypes 会话类型数组 {@link Conversation.ConversationType} 。
     ...
     * <p><Strong>Note: Chat rooms are not included and messages in chat rooms are not counted.</Strong></p>
     *
     * @param conversationTypes An array of conversation types {@link Conversation.ConversationType}.
    ...
    /**
     * /~english
     * Set the provider of the user information for the RongIM call to get the user name and portrait information After setting, when the user information is displayed in the sdk interface, the UserDataProvider.UserInfoProvider#getUserInfo(String) user will be called back and only shall provide the corresponding user information according to the corresponding userId. If you shall get user information asynchronously from the server, the consumer can initiate an asynchronous request in this method and then return null information. After the result of the asynchronous request is returned, refreshUserInfoCache(UserInfo) is called to refresh the user information according to the returned result.
     *
     ...
     *                         If the UserInfoProvider provided by App. Each time you shall request user data over the network instead of caching it locally, which will affect the loading speed of user information.
    ...
    /**
     * /~english
     * Register the conversation list message template provider
     *
    ...
    /**
     * /~english
     * Set the conversation notification do not disturb time
     *
    ...
    /**
     * /~chinese
     * Get the conversation notification do not disturb time.
     *
    ...
    /**
     * /~english
     * Remove the conversation notification do not disturb time
     *
    ...
    /**
     * /~english
     * Unregister the registered unread message number change listener
     *
     * @param observer Listeners that receive unread message messages
     */
     ...
     * 发送定向多媒体消息。
     * <p>
     * 向会话中特定的某些用户发送消息，会话中其他用户不会收到消息。<br>
     * 发送前构造 {@link Message} 消息实体，消息实体中的 content 必须为多媒体消息。
     * 例如：{@link ImageMessage} 、{@link FileMessage} 或其他继承自 {@link MediaMessageContent} 的消息。
     * </p>
     *
    ...
    /**
     * /~english
     * Send directed multimedia messages.
     * <p>Messages are sent to specific users in the conversation, and other users in the conversation will not receive the message. <br>
     * The Message message entity is constructed before sending and the contents in the message entity must be multimedia messages.
     * E.g. ImageMessage FileMessage or other message inherited from MediaMessageContent.</p>
     *
     * @param message     The entity that sent the message.
     * @param userIds     Array of directed recipient id.
     * @param pushContent This field is displayed in the notification bar when a remote push message is sent. If you are sending a custom message, this field must be completed, otherwise you will not be able to receive a remote push message. If you send the default message type in SDK, such as RC:TxtMsg, RC:VcMsg, RC:ImgMsg, this field does not need to be filled in and has been specified by default.
     * @param pushData    Push additional information remotely. If this field is set, users can get it by using io.rong.push.notification.PushNotificationMessage#getPushData() when they receive a push message.
     * @param callback    Callback {@link RongIMClient.SendMediaMessageCallback} for sending messages.
     */
     ...
     * 此方法只能发送非多媒体消息，多媒体消息如{@link ImageMessage} {@link FileMessage} 或其他继承自 {@link MediaMessageContent} 的消息须调用
     * {@link #sendDirectionalMediaMessage(Message, String[], String, String, IRongCallback.ISendMediaMessageCallback)} <br>
     ...
     * <p>This method is used to send messages to some of the users in groups and discussion groups, and other users will not receive this message. The status and body of the sent message are called back by the method in IRongCoreCallback.ISendMessageCallback.
     * This method can only be used to send non-multimedia messages. For multimedia messages such as ImageMessage FileMessage or other messages inherited from MediaMessageContent, you must call sendDirectionalMediaMessage(Message, String[], String, String, IRongCoreCallback.ISendMediaMessageCallback)
     * . If you use IMLib, you can use this method to send directed messages; and if you use IMKit, you can use the method of the same name in RongIM to send directed messages, otherwise UI will not be updated automatically.</p>
     *
     * @param type        Conversation type
     ...
     * @param pushContent This field is displayed in the notification bar when a remote push message is sent. If you are sending a custom message, this field must be completed, otherwise you will not be able to receive a remote push message. If you send the default message type in SDK such as RC:TxtMsg, RC:VcMsg, RC:ImgMsg, this field does not need to be filled in and has been specified by default.
     * @param pushData    Push additional information remotely. If this field is set, users can get it by using io.rong.push.notification.PushNotificationMessage#getPushData() when they receive a push message.
     * @param userIds     A list of users in a discussion group or group conversation that will receive this message.
     * @param callback    Callback for sending messages, refer to IRongCoreCallback.ISendMessageCallback.
     */
     ...
     * 发送地理位置消息。
     * <p>
     * 发送前构造 {@link Message} 消息实体，消息实体中的 content 必须为 {@link LocationMessage}, 否则返回失败。<br>
     ...
     * @group 消息操作
     * @deprecated {@link RongIMClient#sendMessage(Message, String, String, SendMessageOption, IRongCallback.ISendMessageCallback) }
     */

    /**
     * /~english
     * Send a geolocation message. And update the interface at the same time.
     * The Message message entity is constructed before sending and the content in the message entity must be LocationMessage, or the return fails.
     * The thumbnail address scheme only supports file:// and http:// but not others.
     *
     * @param message             Message entity.
     * @param pushContent         This field is displayed in the notification bar when a push message is sent. If you are sending a custom message, this field must be completed, otherwise you cannot receive the push message. If you send the default message type in sdk such as RC:TxtMsg, RC:VcMsg, RC:ImgMsg, this field does not need to be filled in and has been specified by default.
     * @param pushData            Push additional information. If this field is set, users can get it through io.rong.push.notification.PushNotificationMessage#getPushData() method when they receive a push message.
     * @param sendMessageCallback Callback for sending messages, refer to IRongCallback.ISendMessageCallback.
     */
     ...
     * <p>The Message message entity is constructed before sending and the contents in the message entity must be multimedia messages.
     * E.g. ImageMessage FileMessage or other message inherited from MediaMessageContent.</p>
     *
     * @param message     The entity that sent the message.
     * @param pushContent This field is displayed in the notification bar when a remote push message is sent. If you are sending a custom message, this field must be completed, otherwise you will not be able to receive a remote push message. If you send the default message type in sdk such as RC:TxtMsg, RC:VcMsg, RC:ImgMsg, this field does not need to be filled in and has been specified by default.
     * @param pushData    Push additional information remotely. If this field is set, users can get it by using io.rong.push.notification.PushNotificationMessage#getPushData() when they receive a push message.
     * @param callback    Callback io.rong.imlib.RongCoreClient.SendMediaMessageCallback for sending messages.
     */
     ...
     * <p>
     * 上传多媒体文件时，会回调 {@link IRongCallback.ISendMediaMessageCallbackWithUploader#onAttached(Message, IRongCallback.MediaMessageUploader)}
    ...
    /**
     * /~english
     * Send multimedia messages, which can be used to upload multimedia files to your own server
     * <p>When multimedia files are uploaded, IRongCoreCallback.ISendMediaMessageCallbackWithUploader.onAttached(Message, IRongCoreCallback.MediaMessageUploader) will be called back. This callback carries IRongCoreCallback.MediaMessageUploader object and a user only needs to call
     * IRongCoreCallback.MediaMessageUploader.update(int) in this object, update the progress
     * IRongCoreCallback.MediaMessageUploader.success(Uri) and success state, inform the address of successfully uploaded files
     * IRongCoreCallback.MediaMessageUploader.error(), and update failure status
     * IRongCoreCallback.MediaMessageUploader.cancel() ()} Update cancel status </p>
     *
     * @param message     The entity that sent the message.
     * @param pushContent This field is displayed in the notification bar when a remote push message is sent. If you are sending a custom message, this field must be completed, otherwise you will not be able to receive a remote push message. If you send the default message type in SDK, such as RC:TxtMsg, RC:VcMsg, RC:ImgMsg and RC:FileMsg, this field does not need to be filled in and has been specified by default.
     * @param pushData    Push additional information remotely. If this field is set, users can get it by using io.rong.push.notification.PushNotificationMessage#getPushData() when they receive a push message.
     * @param callback    Callback for sending messages, which carries IRongCoreCallback.MediaMessageUploader object and a user calls the method in this object to update the status.
     */
     ...
     * /~english
     * Send a message.
     * <p>The status and body of the sent message are called back by the method in IRongCoreCallback.ISendMessageCallback.
     * <strong>Note: No more than 5 messages can be sent in 1 second.</strong></p>
     *
     * @param type        Conversation type
     * @param targetId    Conversation id. Depending on the conversationType, it may be user id, discussion group id, group id, or chatroom id.
     * @param content     Message content, e.g. TextMessage, ImageMessage.
     * @param pushContent This field is displayed in the notification bar when a remote push message is sent. If you are sending a custom message, this field must be completed, otherwise you will not be able to receive a remote push message. If you send the default message type in sdk such as RC:TxtMsg, RC:VcMsg, RC:ImgMsg, this field does not need to be filled in and has been specified by default.
     * @param pushData    Push additional information remotely. If this field is set, users can get it by using io.rong.push.notification.PushNotificationMessage#getPushData() when they receive a push message.
     * @param callback    Callback for sending messages, refer to {@link IRongCallback.ISendMessageCallback}.
     */
     ...
     * @param pushContent This field is displayed in the notification bar when a remote push message is sent. If you are sending a custom message, this field must be completed, otherwise you will not be able to receive a remote push message. If you send the default message type in sdk such as RC:TxtMsg, RC:VcMsg, RC:ImgMsg, this field does not need to be filled in and has been specified by default.
     * @param pushData    Push additional information remotely. If this field is set, users can get it by using io.rong.push.notification.PushNotificationMessage#getPushData() when they receive a push message.
     * @param option      Additional option for sending messages. Currently you can only set isVoIPPush If the peer device is iOS, isVoIPPush is set as True, Push will be pushed via VoIP channel.
     * @param callback    Callback for sending messages, refer to {@link IRongCallback.ISendMessageCallback}.
     */
    ...
    /**
     * /~english
     * Set the provider of group information
     * After the setting is set, when the group information is displayed in the sdk interface, it will call back UserDataProvider.GroupInfoProvider#getGroupInfo(String) and users only shall provide the corresponding group information according to the corresponding groupId. If you shall get the group information asynchronously from the server, the consumer can initiate an asynchronous request in this method and then return the null information. After the result of the asynchronous request is returned, refreshGroupInfoCache(Group) refresh information is called based on the returned result.
     *
     * @param groupInfoProvider Group information provider
     * @param isCacheGroupInfo  Set whether user information is cached by IMKit.
     *                          If the GroupInfoProvider provided by App. Each time you shall request group data through the network instead of caching the group data locally, which will affect the loading speed of the group information. In this case, it is best to set this parameter to true, and cache the group information by IMKit.
     */
    ...
    /**
     * /~english
     * Set the GroupUserInfo provider for RongIM calls to get GroupUserInfo
     * <p>
     * You can use this method to modify user nicknames in the group
     * <p>
     * After setting, when the user information is displayed in the sdk interface, the UserDataProvider.GroupUserInfoProvider#getGroupUserInfo(String, String) will be called back and the user only shall provide the corresponding user information GroupUserInfo according to the corresponding groupId, userId. If you shall get user information asynchronously from the server, the consumer can initiate an asynchronous request in this method and then return null information. After the result of the asynchronous request is returned, refreshGroupUserInfoCache(GroupUserInfo) refresh information is called based on the returned result.
     *
     ...
     *                              If the GroupUserInfoProvider is provided by App Each time you shall request data over the network instead of caching the data locally, which will affect the loading speed of the information.
    ...
    /**
     * /~english
     * Set the provider of location information.
     *
...
//    }
     ...
     * The '@ 'function and the VoIP function are in the selection interface. You shall know the information about the members in the group, and the developer shall set the provider. The developer shall obtain the group member information in the callback and set it to the sdk through the method in RongMentionManager.IGroupMemberCallback.
     *
    ...
    /**
     * /~english
     * Set voice message sampling rate
     *
    ...
    /**
     * /~english
     * Sampling rate of voice message
     *
    ...
    /**
     * /~english
     * The message status is set according to the Message, and the callback method is used to obtain whether the setting is successful.
     *
    ...
    /**
     * /~english
     * Set the listener for the operation of the official account interface
     *
    ...
    /**
     * /~english
     * Set the public service menu and click listening Recommended method: set listening when entering the corresponding public service conversation When exiting the conversation, reset the listening to null, to prevent memory leaks
     *
    ...
    /**
     * /~english
     * Set the provider of public service account information for RongIM to get the public service account name, portrait information and public service number menu.
     * Currently, the default official account service of sdk shall not be set by developers. This interfaceprovides another way to set public service account information from the app layer. When the user information is displayed in the sdk interface, PublicServiceManager.PublicServiceProfileProvider#getPublicServiceProfile(Conversation.PublicServiceType, String) users only shall provide the corresponding public service account information according to the corresponding publicServiceType, publicServiceId. If you shall obtain the public service account information asynchronously from the server, the consumer can initiate an asynchronous request in this method and then return the null information. After the result of the asynchronous request is returned, refreshPublicServiceProfile(PublicServiceProfile) is called to refresh the official account information according to the returned result.
     *
     * @param publicServiceProfileProvider PublicServiceManager.PublicServiceProfileProvider, the provider of public service account information.
     */
    ...
    /**
     * /~english
     * Set the conversation type for sending message receipts. Currently, only private chats, groups and discussion groups are supported. Private chat is supported by default.
     *
    ...
    /**
     * /~english
     * Set the current user information If the developer does not implement a user information provider, but uses messages to carry user information, you shall use this method to set the current user's information, and then call RongUserInfoManager.getInstance().setMessageAttachedUserInfo(boolean)} afterinit(Application, String, boolean), so that you can carry the current user's information in each message. IMKit will take out the user's information and refresh it to the interface when it receives the message.
     *
    ...
    /**
     * /~english
     * Start a chat room conversation
     * <p>
     * Set the parameter createIfNotExist to true, corresponding interface to be called in kit is RongIMClient#joinChatRoom(String, int, RongIMClient.OperationCallback). If the chat room does not exist, it is automatically created and joined. If the callback fails, warning pops up.
     * <p>
     * Set the parameter createIfNotExist to false, corresponding interface to be called in kit is RongIMClient#joinExistChatRoom(String, int, RongIMClient.OperationCallback) If the chat room does not exist, an error RongIMClient.ErrorCode#RC_CHATROOM_NOT_EXIST is returned.
     *
     * @param context          Application context
     * @param chatRoomId       Chat room id
     ...
     * Start the conversation interface
     * <p>
     * Multiple conversation types Conversation.ConversationType can be passed in to correspond to different conversation types and open different conversation interfaces in use. If Conversation.ConversationType#CHATROOM is passed in, RongIMClient#joinChatRoom(String, int, RongIMClient.OperationCallback) will be called by default to join the chat room. If your logic is that only existing chat rooms can be joined, the interface startChatRoomChat(Context, String, boolean) is used and the third parameter is false.
     *
     * @param context          Application context.
     * @param conversationType Conversation type
     * @param targetId         Depending on the conversationType, it may be user Id, discussion group Id, group Id or chat room Id
    ...
    /**
     * /~english
     * Start the customer service chat interface
     *
     * @param context           Application context
     * @param customerServiceId Customer service Id to chat with
     * @param title             The title of the chat Developers shall get this value through intent.getData (). GetQueryParameter ("title") in the chat interface, and then manually set it as the title of the chat interface
     * @param customServiceInfo The user information of the current customer service user.CSCustomServiceInfo
     */
    ...
    /**
     * /~english
     * A list of a specific type of conversations after starting the aggregation
     * For example, if a single chat conversation is set to aggregate, this method allows you to open a list of all single chat conversations.
     *
     * @param context          Application context
     * @param conversationType Conversation type
     */
